...
mod tests {
    ...
    use propagate::{good, take, Propagate};
    ...
    fn test_anyhow_inner() -> Result<()> {
        ...
        let two: (i32, i32) =
            good!(my_enum => full |v| bail!("Cannot get good value! The value is {:?}", v));
        assert_eq!(two, (0, 1));
    ...
    fn anyhow_eq<T: PartialEq>(res1: Result<T>, res2: Result<T>) -> bool {
        match (res1, res2) {
            (Ok(v1), Ok(v2)) => v1 == v2,
            (Err(err1), Err(err2)) => err1.to_string() == err2.to_string(),
            _ => false,
        }
    }
    #[test]
